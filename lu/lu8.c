// 8: Increased the number of unrolled loop iterations to 16
        ...
        for (j = i + 1; j < N; j++)
        {
            ...
            for (k = i + 1; k < N;)
            {
                if (k + 16 < N)
                {
                    ...
                    tmp4 = _mm256_loadu_pd(A + IDX(j, k+8, N));
                    tmp5 = _mm256_loadu_pd(A + IDX(i, k+8, N));
                    tmp6 = _mm256_loadu_pd(A + IDX(j, k+12, N));
                    ...
                    _mm256_storeu_pd(A + IDX(j, k+8, N), tmp4);
                    _mm256_storeu_pd(A + IDX(j, k+12, N), tmp6);
...
int main(int argc, const char* argv[])
{
    ...
    if (argc == 1)
    {
        sprintf(fileName, "results/lu8_results.txt");
    }
    else
    {
        sprintf(fileName, "results/lu8_results_%s.txt", argv[1]);
    }
