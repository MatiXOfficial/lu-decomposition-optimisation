    ...
    for (i = 0; i < N; i++)
    {
        ...
        for (j = i + 1; j < N; j++)
        {
            ...
            for (k = i + 1; k < N;)
            {
                if (k + 8 < N)
                {
                    tmp0 = _mm_loadu_pd(A + IDX(j, k, N));
                    tmp1 = _mm_loadu_pd(A + IDX(i, k, N));
                    tmp2 = _mm_loadu_pd(A + IDX(j, k+2, N));
                    ...
                    _mm_storeu_pd(A + IDX(j, k, N), tmp0);
                    _mm_storeu_pd(A + IDX(j, k+2, N), tmp2);
                    _mm_storeu_pd(A + IDX(j, k+4, N), tmp4);
                    _mm_storeu_pd(A + IDX(j, k+6, N), tmp6);
...
int main(int argc, const char* argv[])
{
    ...
    if (argc == 1)
    {
        sprintf(fileName, "results/lu6_results.txt");
    }
    else
    {
        sprintf(fileName, "results/lu6_results_%s.txt", argv[1]);
    }
